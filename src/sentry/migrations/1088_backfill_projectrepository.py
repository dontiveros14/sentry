...
def backfill_project_repository(apps: StateApps, schema_editor: BaseDatabaseSchemaEditor) -> None:
    ...
    # Step 2: Create ProjectRepository rows in batches (skip any that already
    # exist from the dual-write phase).
    ...
    # Step 3: Build a lookup of (project_id, repository_id) -> ProjectRepository.id
    ...
    # Step 4: Backfill project_repository_id on RepositoryProjectPathConfig.
    # Use .update() instead of .save() to avoid firing the post_save signal
    # which would spawn an update_code_owners_schema task for every row.
    ...
    # Step 5: Backfill project_repository_id on SeerProjectRepository
